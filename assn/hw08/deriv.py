	...
	if isinstance(m1, const):
		if isinstance(m2, const):
			...
			if isinstance(m2.get_mult1(), const):
				return make_prod(make_const(m1.get_val() * m2.get_mult1().get_val()), deriv(m2.get_mult2()))
			elif isinstance(m2.get_mult2(), const):
				return make_prod(make_const(m1.get_val() * m2.get_mult2().get_val()), deriv(m2.get_mult1()))
			return prod(mult1=const(m1.get_val()),
						mult2=deriv(m2))
		elif isinstance(m2, quot) or isinstance(m2, ln):
